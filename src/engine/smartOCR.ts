...
function normalizeKey(p: PatientEntry): string | null {
  ...
  // Normalize room: strip all separators (49-1, 49/1, 49.1 → all become "491").
  // Raw room is fragile — OCR and manual paste produce different separator chars.
  // mergeScan uses the same normalization via buildPatientKey, so this must match.
  const normalizedRoom = p.room
    ? p.room.replace(/[^\u05D0-\u05EAa-z0-9]/gi, "").toLowerCase()
    : "";
  if (normalizedRoom) {
